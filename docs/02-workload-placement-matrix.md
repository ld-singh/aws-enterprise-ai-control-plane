...
- Amazon Bedrock, Amazon Bedrock AgentCore, Amazon EKS, Amazon SageMaker AI, and AWS ParallelCluster are workload platforms or managed service boundaries.
- AWS Trainium, AWS Inferentia, and NVIDIA GPUs are accelerator choices used through one or more workload platforms.
...
Adopt a portfolio placement model rather than forcing all AI workloads onto one platform. Amazon Bedrock is the default for managed foundation model consumption, Amazon Bedrock AgentCore is the reference path for managed agent runtime capabilities, Amazon EKS supports custom serving and platform control, Amazon SageMaker AI and Amazon SageMaker HyperPod support managed ML lifecycle and large-scale training needs, and AWS ParallelCluster + Slurm supports HPC workloads. Accelerator selection is a separate decision based on compatibility, performance, availability, portability, and total cost.
...
1. Start with Amazon Bedrock when an approved managed model meets functional, security, regional, and economic requirements.
2. Use Amazon Bedrock AgentCore when agent runtime capabilities materially reduce platform work; do not use an agent where a deterministic workflow is safer and easier to operate.
3. Move to Amazon EKS-based serving only when model choice, runtime control, portability, or measured economics justify owning the platform.
4. Use Amazon SageMaker AI for managed ML lifecycle requirements and evaluate Amazon SageMaker HyperPod for sustained, large-scale training where training resilience and cluster continuity matter.
5. Use AWS ParallelCluster + Slurm when the workload and users are HPC-oriented. Scheduler familiarity and job semantics are valid architecture inputs.
6. Select AWS Trainium or AWS Inferentia only after compatibility and performance validation on the target model. Select NVIDIA where ecosystem requirements or portability justify it.
...
|---|---|---|
| Time to first controlled workload | Amazon Bedrock, Amazon Bedrock AgentCore, Amazon SageMaker AI | Amazon EKS or Slurm only when a platform already exists |
| Need for custom runtime behavior | Limited | Amazon EKS + vLLM/Triton |
| HPC scheduling and research workflow alignment | Limited | AWS ParallelCluster + Slurm |
| Large-scale managed training environment | Amazon SageMaker HyperPod | Amazon EKS or Slurm when control and existing skills justify ownership |
| Lowest idle-capacity exposure | Consumption-based managed services | Elastic batch clusters with disciplined scale-down |
| Maximum accelerator and software control | Limited | Amazon EKS, Slurm, or dedicated NVIDIA platform |
...
- [ADR-004: Amazon SageMaker HyperPod vs Self-Managed Amazon EKS](decision-records/adr-004-sagemaker-hyperpod-vs-self-managed-eks.md)
- [ADR-005: AWS Trainium/AWS Inferentia vs NVIDIA GPU](decision-records/adr-005-trainium-inferentia-vs-nvidia-gpu.md)
